...
## 🤖 智能 Agent 自动化与多模型支持（全新功能）

Operit AI 现已集成强大的智能 agent 自动化能力：

- **一键自动理解需求、生成/优化/执行脚本**：输入需求后，agent 自动理解、规划、生成并执行脚本，自动反馈和多轮优化。
- **多大模型（LLM）支持**：可在界面选择 OpenAI、Qwen、Claude 等主流大模型，支持自定义 API Key。
- **agent 配置界面**：支持 UI 上热切换 LLM、设置最大优化轮数、API Key 等参数。
- **任务中断与回滚**：agent 执行过程中可随时中断，历史脚本可一键回滚。
- **历史脚本管理**：所有 agent 生成/优化的脚本自动保存，支持查看、回滚、删除。
- **每轮过程可视化**：UI 实时展示 agent 每轮计划、脚本、执行结果、LLM 评价，便于追踪和调试。

> 体验入口：主界面底部输入区，点击 agent 按钮即可体验全流程自动化。
